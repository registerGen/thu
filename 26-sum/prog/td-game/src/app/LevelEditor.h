...
/// set economy/towers/waves, then save/load/play. Supports multiple paths.
...
public:
  /// Serialize the editor state to a level JSON string (for play/save).
  std::string buildLevelJson() const;
  /// Load a level from a JSON string into the editor grid.
...
private:
  ...
  void generateWaves(std::vector<Wave>& waves, int difficulty) const;
  /// Regenerate waves for the current preset (called when the path count
  /// changes, since spawns reference paths by index).
  void regenerateWaves();
  /// Serialize waves into the JSON shown in the waves editor.
  QString wavesToJson(std::vector<Wave> const& waves) const;
  /// Parse waves from the waves editor. Returns false and sets `err` on failure.
  bool extractWaves(std::vector<Wave>& waves, QString& err) const;
  void loadFromLevel(Level const& level);
  Level buildLevel() const;  // private — buildLevelJson serializes this.
  ...
  /// RNG for assigning spawns to paths; mutable so generateWaves (const) can use it.
