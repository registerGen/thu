...
StartMenu::StartMenu(std::vector<LevelInfoView> infos, QWidget* parent) : QWidget(parent) {
...
void StartMenu::buildLevelRows(std::vector<LevelInfoView> const& infos) {
  int display_row = 0;
...
void TowerPalette::rebuild() {
  ...
  int idx = 0;
  for (auto const& kind : controller_->levelView().available_towers) {
    int cost = 0;
    for (auto const& tc : controller_->levelView().tower_costs) {
      if (tc.kind == kind) {
        cost = tc.cost;
        break;
      }
    ...
    theme::drawTowerPreview(ip, kind, QRectF(4, 4, 32, 32));
    ip.end();

    auto* btn = new QPushButton;
    btn->setIcon(icon);
    btn->setIconSize(QSize(32, 32));
    btn->setText(QString::fromStdString(kind) + " (" + QString::number(cost) + ")");
    ...
    connect(btn, &QPushButton::clicked, this, [this, kind, btn] {
      if (btn->isChecked()) {
        for (auto* b : group_->buttons())
          if (b != btn) static_cast<QPushButton*>(b)->setChecked(false);
        emit typeSelected(QString::fromStdString(kind));
      } else {
...
Hud::Hud(GameController* controller, QWidget* parent) : QWidget(parent), controller_(controller) {
  ...
  connect(controller_, &GameController::stateChanged, this, [this] {
    if (controller_->state() != GameState::Playing) setControlsVisible(false);
  });
...
void Hud::refresh() {
  resources_->setText(QString::number(controller_->resourceAmount()));
  wave_->setText(QString::number(controller_->currentWave()));
  int idx = controller_->levelIndex();
  level_->setText(
    idx >= 1 ? QString("%1. %2").arg(idx).arg(QString::fromStdString(controller_->levelName()))
             : QString("Custom: %1").arg(QString::fromStdString(controller_->levelName()))
  );
  score_->setText(QString::number(controller_->score()));
  time_->setText(QString::number(controller_->elapsedTime(), 'f', 1) + "s");
  fps_->setText(QString::number(controller_->fps(), 'f', 0));
  pause_btn_->setText(controller_->paused() ? "Resume" : "Pause");
}
