...
std::string NormalBullet::type() const { return "normal"; }
void NormalBullet::impact(Enemy& enemy) const { enemy.decrease_health(health_damage_); }
...
std::string SlowBullet::type() const { return "slow"; }
void SlowBullet::impact(Enemy& enemy) const {
...
std::string PoisonBullet::type() const { return "poison"; }
void PoisonBullet::impact(Enemy& enemy) const {
  enemy.apply_status_effect(std::make_unique<PoisonEffect>(poison_effect_));
}

SplashBullet::SplashBullet(Vec2 position, Vec2 velocity, float radius, int health_damage)
    : Bullet(position, velocity), radius_(radius), health_damage_(health_damage) { }

std::string SplashBullet::type() const { return "splash"; }
float SplashBullet::radius() const { return radius_; }
bool SplashBullet::effective(Enemy const& enemy) const {
  return enemy.position().distance(position()) <= radius_;
}

void SplashBullet::impact(Enemy& enemy) const {
  enemy.decrease_health(
    std::max(1, static_cast<int>(static_cast<float>(health_damage_) * enemy.splash_damage_factor()))
  );
}

bool SplashBullet::pierces() const { return true; }
...
std::string LaserBullet::type() const { return "laser"; }
bool LaserBullet::effective(Enemy const& enemy) const {
  ...
  Vec2 bullet_to_enemy = enemy.position() - position();
  // Check if the enemy is in front of the bullet (dot product >= 0)
  if (bullet_to_enemy.dot(bullet_direction) < 0.0f) return false;

  float distance_to_line = std::abs(bullet_to_enemy.cross(bullet_direction));
  return distance_to_line <= width_ / 2.0f;
}

void LaserBullet::impact(Enemy& enemy) const { enemy.decrease_health(health_damage_); }
