...
Enemy::Enemy(
  ...
  float tower_damage_interval
    ...
    : type_(type),
      ...
      tower_damage_interval_(tower_damage_interval) { }
Enemy::~Enemy() = default;
...
void Enemy::increase_health(int amount) {
  health_ += amount;
...
float Enemy::tower_damage_interval() const { return tower_damage_interval_; }
int Enemy::score() const { return score_; }
...
void Enemy::update(Game& game, float dt) {
  ...
  float velocity_factor = terrain_speed_factor * std::min(1.0f, slow_mult * slow_resist_factor());
...
ResistantEnemy::ResistantEnemy(
  ...
  float splash_resist
    ...
    : Enemy(type, bounds, speed, health, score, tower_damage, tower_damage_interval),
...
void SplitterEnemy::on_death(Game& game) {
  for (int i = 0; i < child_spec_.count; ++i) {
    auto child = std::make_shared<Enemy>(
      ...
      child_spec_.tower_damage_interval
    );
    ...
    if (child_spec_.perturbation > 0.0f) {
      ...
      if (path_ && dist > path_->total_length() - 0.01f)
