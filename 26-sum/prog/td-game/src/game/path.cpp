...
Path::Path(
  ...
  std::vector<std::pair<std::size_t, std::size_t>> portal_pairs
    ...
    : waypoints_(waypoints), portal_pairs_(std::move(portal_pairs)) {
...
std::vector<std::pair<std::size_t, std::size_t>> const& Path::portal_pairs() const {
  return portal_pairs_;
}

float Path::paired_portal_distance(Vec2 position) const {
  for (auto const& [src, tgt] : portal_pairs_) {
    if (waypoints_[src].x == position.x && waypoints_[src].y == position.y) {
      return cumulative_[tgt];
    }
  }
  return -1.0f;
}
