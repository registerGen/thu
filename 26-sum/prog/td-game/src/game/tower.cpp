...
void Tower::increase_health(int amount) {
  health_ += amount;
...
int Tower::effective_resource_cost() const {
  return std::max(
    1,
...
void Tower::apply_tower_damage(std::shared_ptr<Enemy> enemy) {
  ...
  tower_damage_.emplace_back(enemy->tower_damage(), enemy->tower_damage_interval(), enemy);
}
...
std::unique_ptr<Bullet> AttackTower::release_bullet(Vec2 target) {
  return make_bullet_(tile()->position(), target);
}
...
void AttackTower::update(Game& game, float dt) {
  ...
  if (can_attack()) {
    if (Enemy* target = select_target(game.enemies())) {
      ...
      Vec2 predicted = target->predicted_position(lead);
      if (speed > 0.0f) {
        lead = (predicted - src).length() / speed;
        predicted = target->predicted_position(lead);
      }
      aim_ = (predicted - src).normalized();
      if (aim_.length_sq() == 0.0f) aim_ = {1.0f, 0.0f};

      game.spawn_bullet(release_bullet(predicted));
