...
public:
  ...
  int effective_resource_cost() const;
  void apply_tower_damage(std::shared_ptr<Enemy> enemy);

  /// Whether this tower may be placed on the given tile.
  bool can_place_on(Tile const& tile) const;
...
private:
  ...
  std::unique_ptr<Bullet> release_bullet(Vec2 target);
