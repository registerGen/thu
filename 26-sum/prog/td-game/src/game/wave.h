...
/// One enemy spawn within a wave: an enemy type at a relative time on a path.
/// `path` is mandatory — every spawn must state which path the enemy follows.
struct EnemySpawn {
  std::string type;
  float time;
  int path;
...
struct Wave {
  float gap = 0.0f;  // pre-delay before this wave begins
...
/// and the per-path paths. Built by Game from the current Level.
struct WaveSpec {
  std::vector<Wave> const* waves = nullptr;
  EnemyStatsTable const* enemy_stats = nullptr;
  std::vector<Path const*> paths;  // path index -> Path the spawned enemy follows
};
