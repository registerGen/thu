...
float path_distance_at(Level const& lvl, Vec2 pos) {
  auto const& wps = lvl.paths[0].waypoints();
  float cum = 0.0f;
...
TEST_CASE("place_tower validation", "[game][placement]") {
  ...
  SECTION("valid off-path tile succeeds") {
    REQUIRE(game.place_tower("normal", Vec2{0.5f, 0.5f}) == 0);  // grass, row 0 col 0
    REQUIRE(game.towers().size() == 1);
  }
  SECTION("towers may be placed on the path (they block enemies)") {
    REQUIRE(game.place_tower("normal", Vec2{5.5f, 3.5f}) == 0);  // path row 3 col 5
    REQUIRE(game.towers().size() == 1);
  }
  SECTION("rock tile rejected") {
    REQUIRE(game.place_tower("normal", Vec2{1.5f, 2.5f}) == 1);  // rock: not placeable
    REQUIRE(game.towers().empty());
  }
  SECTION("wall can be placed on the path") {
    REQUIRE(game.place_tower("wall", Vec2{5.5f, 3.5f}) == 0);
    REQUIRE(game.towers().size() == 1);
  }
  SECTION("unavailable tower type rejected") {
    // Level 1 does not offer splash/laser/poison.
    REQUIRE(game.place_tower("laser", Vec2{0.5f, 0.5f}) == 1);  // not placeable
    REQUIRE(game.towers().empty());
  }
  SECTION("insufficient resources rejected") {
    // Starting resources 150; normal costs 50 -> can place 3, 4th fails.
    REQUIRE(game.place_tower("normal", Vec2{0.5f, 0.5f}) == 0);
    REQUIRE(game.place_tower("normal", Vec2{2.5f, 0.5f}) == 0);
    REQUIRE(game.place_tower("normal", Vec2{4.5f, 0.5f}) == 0);
    REQUIRE(game.place_tower("normal", Vec2{6.5f, 0.5f}) == 2);  // insufficient resources
  }
  SECTION("reject when game is paused") {
    game.pause();
    REQUIRE(game.place_tower("normal", Vec2{0.5f, 0.5f}) == 1);  // not placeable
    game.resume();
    REQUIRE(game.place_tower("normal", Vec2{0.5f, 0.5f}) == 0);  // now allowed
  }
}
...
TEST_CASE("Attack tower fires a bullet aimed at an in-range enemy", "[game][targeting]") {
  GameFixture fx(config_dir());
  Game& game = fx.game;
  REQUIRE(game.place_tower("normal", Vec2{4.5f, 2.5f}) == 0);  // grass, row 2 col 4, near path
  ...
  auto enemy = make_enemy("normal", game.level().enemy_stats.at("normal"), &game.level().paths[0]);
...
TEST_CASE("Wall blocks enemies and takes damage", "[game][wall]") {
  GameFixture fx(config_dir());
  Game& game = fx.game;
  REQUIRE(game.place_tower("wall", Vec2{5.5f, 3.5f}) == 0);  // on the path (row 3 col 5)
...
TEST_CASE("Any tower on the path blocks enemies and takes damage", "[game][block]") {
  ...
  REQUIRE(game.place_tower("normal", Vec2{5.5f, 3.5f}) == 0);  // on path, col 5 row 3 (distance ~6)
  auto enemy =
    make_enemy("armored", game.level().enemy_stats.at("armored"), &game.level().paths[0]);
...
TEST_CASE("Portal teleports enemies forward", "[game][portal]") {
  ...
  REQUIRE_FALSE(game.level().paths[0].portal_pairs().empty());

  auto [src_idx, tgt_idx] = game.level().paths[0].portal_pairs().front();
  Vec2 portal_pos = game.level().paths[0].waypoints()[src_idx];
  float source_dist = path_distance_at(game.level(), portal_pos);
  REQUIRE(source_dist >= 0.0f);
  float dest_dist = game.level().paths[0].paired_portal_distance(portal_pos);
...
TEST_CASE("Cheats", "[game][cheat]") {
  ...
  SECTION("killall clears enemies") {
    ...
    auto e = make_enemy("normal", game.level().enemy_stats.at("normal"), &game.level().paths[0]);
    game.spawn_enemy(e);
...
TEST_CASE("Splitter spawns children on death", "[game][splitter]") {
  ...
  auto sp = make_enemy("splitter", stats, &game.level().paths[0]);
...
TEST_CASE("Boss regenerates health over time", "[game][boss]") {
  ...
  auto boss = make_enemy("boss", stats, &game.level().paths[0]);
  boss->set_health(100);
...
TEST_CASE("Killing an enemy awards its per-type score", "[game][score]") {
  ...
  auto enemy = make_enemy("normal", stats, &game.level().paths[0]);
  enemy->set_health(1);
  game.spawn_enemy(enemy);
  ...
  Game& g2 = fxg2.game;
  auto boss = make_enemy("boss", g2.level().enemy_stats.at("boss"), &g2.level().paths[0]);
  boss->set_health(1);
  g2.spawn_enemy(boss);
...
TEST_CASE("Resource tower generates gold over time", "[game][integration][resource]") {
  ...
  Game& game = fx.game;
  use_test_level(fx);
  REQUIRE(game.place_tower("resource", Vec2{2.5f, 2.5f}) == 0);  // costs 50 -> 1000 - 50 = 950
...
TEST_CASE("Splash tower damages a clustered group", "[game][integration][splash]") {
  ...
  Game& game = fx.game;
  use_test_level(fx);
  REQUIRE(game.place_tower("splash", Vec2{2.5f, 2.5f}) == 0);  // adjacent to row 3
...
TEST_CASE("Laser pierces multiple enemies in its beam", "[game][integration][laser]") {
  ...
  Game& game = fx.game;
  use_test_level(fx);
  REQUIRE(game.place_tower("laser", Vec2{2.5f, 2.5f}) == 0);
...
TEST_CASE("Poison deals one-shot damage and roots the enemy", "[game][integration][poison]") {
  ...
  Game& game = fx.game;
  use_test_level(fx);
  REQUIRE(game.place_tower("poison", Vec2{2.5f, 2.5f}) == 0);

  auto e = make_enemy("armored", game.level().enemy_stats.at("armored"), &game.level().paths[0]);
  e->set_path_distance(1.5f);
  game.spawn_enemy(e);

  // Armored has 100 hp; poison deals 20 once on application.
  run_until(game, [&] { return e->health() < 100; }, 90);
  REQUIRE(e->health() == 80);
...
TEST_CASE("Slow tower applies a slow status and impedes progress", "[game][integration][slow]") {
  ...
  Game& game = fx.game;
  use_test_level(fx);
  REQUIRE(game.place_tower("slow", Vec2{2.5f, 2.5f}) == 0);

  auto slowed = make_enemy("normal", game.level().enemy_stats.at("normal"), &game.level().paths[0]);
  slowed->set_path_distance(1.5f);
  game.spawn_enemy(slowed);

  run_until(game, [&] { return slowed->status_hint().slow; }, 90);
  ...
  Game& g2 = fxg2.game;
  use_test_level(fxg2);
  auto free = make_enemy("normal", g2.level().enemy_stats.at("normal"), &g2.level().paths[0]);
...
TEST_CASE("Resistant enemy reduces splash damage", "[game][integration][resistant]") {
  ...
  Game& game = fx.game;
  use_test_level(fx);
  REQUIRE(game.place_tower("splash", Vec2{2.5f, 2.5f}) == 0);

  auto r =
    make_enemy("resistant", game.level().enemy_stats.at("resistant"), &game.level().paths[0]);
  r->set_path_distance(1.5f);
  game.spawn_enemy(r);

  run_until(game, [&] { return r->health() < 50; }, 90);
...
TEST_CASE("Misc query API", "[game][query]") {
  ...
  // take_events: fires on the next discrete event.
  REQUIRE(game.place_tower("normal", Vec2{0.5f, 0.5f}) == 0);
  auto events = game.take_events();
  REQUIRE_FALSE(events.empty());
  REQUIRE(events[0].kind == "tower_placed");  // tower placed
}

TEST_CASE("Event queue receives discrete game events", "[game][events]") {
  GameFixture fx(config_dir());
  Game& game = fx.game;

  REQUIRE(game.place_tower("normal", Vec2{4.5f, 2.5f}) == 0);  // near the level-1 path
  auto events = game.take_events();
  bool saw_placed = false;
  for (auto const& e : events) {
    if (e.kind == "tower_placed") saw_placed = true;
  }
  REQUIRE(saw_placed);
  ...
  bool saw_wave = false, saw_killed = false;
  for (auto const& e : events) {
    if (e.kind == "wave_started") saw_wave = true;
    if (e.kind == "enemy_killed") saw_killed = true;
  }
  REQUIRE(saw_wave);
  REQUIRE(saw_killed);
}
...
TEST_CASE("Tower query and health-mutator API", "[game][tower][query]") {
  ...
  Game& game = fx.game;
  use_test_level(fx);
  REQUIRE(game.place_tower("normal", Vec2{2.5f, 2.5f}) == 0);
  REQUIRE(game.place_tower("resource", Vec2{4.5f, 2.5f}) == 0);
  ...
  for (auto const& t : game.towers()) {
    ...
    REQUIRE(t->health() == std::min(15, t->max_health()));
  }
  for (auto const& t : game.towers()) {
...
TEST_CASE("A destroyed tower is removed from the board", "[game][tower][death]") {
  ...
  Game& game = fx.game;
  use_test_level(fx);
  REQUIRE(game.place_tower("wall", Vec2{5.5f, 3.5f}) == 0);  // on the path
  ...
  run_until(game, [&] { return game.towers().empty() || game.over(); }, 60 * 5);
...
TEST_CASE("Splitter clamps children that would overshoot the exit", "[game][splitter][clamp]") {
  ...
  // Fields: width, height, speed, health, score, tower_damage, tower_damage_interval, count,
  // perturbation.
  SplitterEnemy::ChildSpec spec{0.4f, 0.4f, 1.0f, 1, 1, 0, 0.0f, 1, kPerturbation};
...
// ===========================================================================
// View producer tests — the flat POD snapshots the app/ renders from.
// ===========================================================================

TEST_CASE("tower_views reflects placed towers", "[game][views]") {
  GameFixture fx(config_dir());
  Game& game = fx.game;
  REQUIRE(game.place_tower("normal", Vec2{0.5f, 0.5f}) == 0);  // grass, row 0 col 0

  auto views = game.tower_views();
  REQUIRE(views.size() == 1);
  REQUIRE(views[0].kind == "normal");  // normal
  REQUIRE(views[0].health == views[0].max_health);
  REQUIRE(views[0].pos.x == Approx(0.5f));
  REQUIRE(views[0].pos.y == Approx(0.5f));
  // Normal is an attack tower — aim should be a unit vector.
  REQUIRE(views[0].aim.length_sq() == Approx(1.0f).margin(1e-5f));
}

TEST_CASE("enemy_views reflects spawned enemies", "[game][views]") {
  GameFixture fx(config_dir());
  Game& game = fx.game;
  auto enemy = make_enemy("normal", game.level().enemy_stats.at("normal"), &game.level().paths[0]);
  game.spawn_enemy(enemy);
  game.update(1.0f / 60.0f);  // flush into enemies_

  auto views = game.enemy_views();
  REQUIRE(views.size() == 1);
  REQUIRE(views[0].kind == "normal");  // normal
  REQUIRE(views[0].health == views[0].max_health);
  REQUIRE(views[0].half_width > 0.0f);
  REQUIRE(views[0].half_height > 0.0f);
  REQUIRE_FALSE(
    (views[0].status_hint.slow || views[0].status_hint.poison || views[0].status_hint.regen)
  );  // no status effects
}

TEST_CASE("bullet_views reflects spawned bullets", "[game][views]") {
  GameFixture fx(config_dir());
  Game& game = fx.game;
  game.spawn_bullet(std::make_unique<NormalBullet>(Vec2{1.0f, 1.0f}, Vec2{5.0f, 0.0f}, 10));
  game.update(1.0f / 60.0f);

  auto views = game.bullet_views();
  REQUIRE(views.size() == 1);
  REQUIRE(views[0].kind == "normal");  // normal
  REQUIRE(views[0].radius == 0.0f);    // non-splash
  REQUIRE(views[0].vel.x == Approx(5.0f));
}

TEST_CASE("level_view has correct map and path data", "[game][views]") {
  GameFixture fx(config_dir());
  Game& game = fx.game;
  auto lv = game.level_view();

  REQUIRE(lv.map_width > 0.0f);
  REQUIRE(lv.map_height > 0.0f);
  REQUIRE(lv.terrain.size() == static_cast<std::size_t>(lv.map_width * lv.map_height));
  REQUIRE_FALSE(lv.path_waypoints.empty());
  REQUIRE_FALSE(lv.path_waypoints[0].empty());
  REQUIRE_FALSE(lv.available_towers.empty());
  REQUIRE(lv.tower_costs.size() == lv.available_towers.size());
  // Every tower cost entry has a matching available tower type.
  for (auto const& tc : lv.tower_costs) {
    REQUIRE(
      std::find(lv.available_towers.begin(), lv.available_towers.end(), tc.kind) !=
      lv.available_towers.end()
    );
    REQUIRE(tc.cost > 0);
  }
}

TEST_CASE("take_events reports tower placement and enemy kill", "[game][views][events]") {
  GameFixture fx(config_dir());
  Game& game = fx.game;

  // Place a tower -> TowerPlaced event.
  REQUIRE(game.place_tower("normal", Vec2{0.5f, 0.5f}) == 0);
  auto events = game.take_events();
  REQUIRE(events.size() >= 1);
  bool saw_placed = false;
  for (auto const& e : events) {
    if (e.kind == "tower_placed") {  // tower placed
      saw_placed = true;
      REQUIRE(e.a > 0);                 // cost
      REQUIRE(e.type_tag == "normal");  // normal
    }
  }
  REQUIRE(saw_placed);
  REQUIRE(game.take_events().empty());  // drained
  ...
  game.update(1.0f / 60.0f);  // flush into enemies_
  enemy->set_health(0);
  game.update(1.0f / 60.0f);  // reap + event

  events = game.take_events();
  bool saw_killed = false;
  for (auto const& e : events) {
    if (e.kind == "enemy_killed") {  // enemy killed
      saw_killed = true;
      REQUIRE(e.a > 0);  // score
    }
  }
...
TEST_CASE("take_events reports wave start", "[game][views][events]") {
  GameFixture fx(config_dir());
  Game& game = fx.game;
  // Run past the first gap (level 1 gap[0] = 3.0s) so wave 0 starts.
  float dt = 1.0f / 60.0f;
  for (int i = 0; i < 60 * 5 && !game.over(); ++i) game.update(dt);

  auto events = game.take_events();
  bool saw_wave = false;
  for (auto const& e : events) {
    if (e.kind == "wave_started") {  // wave started
      saw_wave = true;
      REQUIRE(e.a >= 1);  // wave number (1-based)
    }
  }
  REQUIRE(saw_wave);
}

TEST_CASE("can_place_at validates tiles", "[game][views]") {
  GameFixture fx(config_dir());
  Game& game = fx.game;

  REQUIRE(game.can_place_at(Vec2{0.5f, 0.5f}));          // grass — placeable
  REQUIRE_FALSE(game.can_place_at(Vec2{1.5f, 2.5f}));    // rock — not placeable
  REQUIRE_FALSE(game.can_place_at(Vec2{-1.0f, -1.0f}));  // out of bounds

  // Occupy a tile with a tower.
  REQUIRE(game.place_tower("normal", Vec2{0.5f, 0.5f}) == 0);
  REQUIRE_FALSE(game.can_place_at(Vec2{0.5f, 0.5f}));  // now occupied
}

TEST_CASE("last_result_view after defeat and victory", "[game][views]") {
  // Defeat: no towers, enemies walk to the exit.
  GameFixture fx(config_dir());
  Game& game = fx.game;
  float dt = 1.0f / 60.0f;
  for (int i = 0; i < 60 * 30 && !game.over(); ++i) game.update(dt);
  REQUIRE(game.state() == Game::State::Defeat);
  auto result = game.last_result_view();
  REQUIRE(result.cleared == false);
  REQUIRE(result.time > 0.0f);

  // Victory: instant-win cheat.
  GameFixture fx2(config_dir());
  Game& g2 = fx2.game;
  g2.apply_cheat("win");
  g2.update(dt);  // triggers the victory check
  REQUIRE(g2.state() == Game::State::Victory);
  auto result2 = g2.last_result_view();
  REQUIRE(result2.cleared == true);
}
