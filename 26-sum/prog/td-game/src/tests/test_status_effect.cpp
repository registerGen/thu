...
TEST_CASE("RegenerationEffect heals over time", "[status][regen]") {
  Path path = straight_path();
  auto enemy = make_path_enemy(path, 1.0f, 60);
  enemy->set_health(40);
  REQUIRE(enemy->health() == 40);

  enemy->apply_status_effect(std::make_unique<RegenerationEffect>(10, 1.0f));  // 10 hp/s for 1s
  enemy->update(default_game(), 0.5f);
  REQUIRE(enemy->health() == 45);  // +5
  enemy->update(default_game(), 0.5f);
  REQUIRE(enemy->health() == 50);  // +5 more

  enemy->update(default_game(), 1.0f);  // expired
  REQUIRE(enemy->health() == 50);

  enemy->apply_status_effect(std::make_unique<RegenerationEffect>(20, 1.0f));  // 20 hp/s for 1s
  enemy->update(default_game(), 0.5f);
  REQUIRE(enemy->health() == 60);  // +10
  enemy->update(default_game(), 0.5f);
  REQUIRE(enemy->health() == 60);  // cannot exceed max health
}
