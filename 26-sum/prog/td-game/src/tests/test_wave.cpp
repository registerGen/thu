...
TEST_CASE("No enemies spawn during the initial pre-wave gap", "[wave][gap]") {
  ...
  Game& game = fx.game;
  float gap0 = game.level().waves[0].gap;  // e.g. 2.0s for level 1
...
TEST_CASE("Enemies appear after the initial gap elapses", "[wave][gap]") {
  GameFixture fx(config_dir());
  Game& game = fx.game;
  float gap0 = game.level().waves[0].gap;
...
TEST_CASE("Gaps are non-uniform between waves", "[wave][gaps]") {
  GameFixture fx(config_dir());
  Game& game = fx.game;
  auto const& waves = game.level().waves;

  REQUIRE(waves.size() >= 3);
  bool any_diff = false;
  for (std::size_t i = 1; i < waves.size(); ++i) {
    if (waves[i].gap != waves[0].gap) any_diff = true;
  }
  REQUIRE(any_diff);
}
...
TEST_CASE("Wave progression is independent of enemy state", "[wave][independent]") {
  ...
  float dt = 1.0f / 60.0f;
  float gap0 = game.level().waves[0].gap;
  float gap1 = game.level().waves.size() > 1 ? game.level().waves[1].gap : 3.0f;
